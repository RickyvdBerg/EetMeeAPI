...
module.exports = {
    postParticipantToMeal(req, res, next) {
        ...
        db.query('SELECT * FROM deelnemers WHERE UserID = (SELECT ID from user where Email = ?) AND StudentenhuisID = ? AND MaaltijdID = ?',
            ...
            (error, result) => {
                if (!result[0]) {
                    db.query('INSERT INTO deelnemers (UserID, StudentenhuisID, MaaltijdID) VALUES ((SELECT ID from user where Email = ?),?,?)',
                        [email, houseId, mealId],
                        (errorInner, resultInner) => {
                            if (!resultInner) {
                                res.status(404).json({ "error": "one or more params incorrect " + result })
                                return
                            }
